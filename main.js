...
const newLink = (link, isExternal) => {
  ...
  linkTo.style =
    "background-color: var(--color-page-header-bg); padding: 1rem 1.5rem; padding-left: 2rem; text-decoration: none; display: flex; align-items:center; gap: 0.6rem;"
  linkTo.innerHTML = `<strong>Ghpage</strong>
...
if(siteName === 'github.com' && gitRepoName && (gitHubPageReference || repoLinkReference.nextElementSibling)) {
  ...
  {
    (!isHasExternalLink || isContainName) ? newLink(gitRepoName) : newLink(`${repoLinkReference.nextElementSibling.firstElementChild.textContent}`, true)
  } else if (isContainName && !isTheMainRepo || !isContainName && !isTheMainRepo) {
    (!isHasExternalLink) ? newLink(`${url[1]}.github.io/${url[2]}`) : newLink(`${repoLinkReference.nextElementSibling.firstElementChild.textContent}`, true)
  }
}